...
int input_read_parameters(
  ...
  } //flag, convert to Mpc was here before
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_dcdmdr:
    case Omega_dm_tot: //class_dcdm_sr modification
      ...
      */
      ...
      // printf("x = Omega_ini_guess = %.8e, dxdy = %g id = %d\n",xguess[index_guess],*dxdy,index_guess); //class_dcdm_sr modification
      break;
    case omega_dcdmdr:
      Omega_M = ba.Omega0_cdm+ba.Omega0_dcdmdr+ba.Omega0_b;
      /* This formula is exact in a Matter + Lambda Universe, but only
...
int input_auxillary_target_conditions(struct file_content * pfc,
  ...
  */
  ...
  case Omega_ini_dcdm:
  case omega_ini_dcdm:
  case omega_ini_dcdm_hat: //class_dcdm_sr modification
  case f_dm_decay: //class_dcdm_sr modification
